import {toEnglish} from "./formatHelpers";
...
export const searchString = (data, name_string, searchValue) => {
    let temp, flag;
    temp = data.filter(function (item) {
        if (item.hasOwnProperty("first_name") && item.hasOwnProperty("last_name")) {
            var temp_name = item.first_name;
            item.first_name += " " + item.last_name;
        }
        flag = false;
        name_string.forEach(function (field) {
            if (item[field] != null)
                if (toEnglish(item[field])
                    .replace(/\s/g, '')
                    .toUpperCase()
                    .includes(toEnglish(searchValue)
                        .replace(/\s/g, '')
                        .toUpperCase()))
                    flag = true;
        });
        if (item.hasOwnProperty("first_name") && item.hasOwnProperty("last_name"))
            item.first_name = temp_name;
        return flag;
    });
    return temp;
}

export const fetchResults = async (limit, page = 1, api, column, order, lastSearch ="", searchIn = []) => {
    lastSearch = lastSearch.trim();

    const offset = (page - 1) * limit;
     let url = "http://localhost:3000/api/"+api+"?offset="+offset+"&limit="+limit+"&column="+column+"&order="+order;

    if(searchIn.length > 0 && lastSearch != "") {
        const searchCols = searchIn.join(",");
        url += "&search=" + lastSearch + "&searchcol=" + searchCols;
    }
...
export const fetchCount = async ( api, lastSearch = "", searchIn = []) => {
    lastSearch = lastSearch.trim();
    let url = "http://localhost:3000/api/"+api+"?count=true";
