...
export const logout = async (queryParams = null) => {
  try {
    const url = queryParams
      ? craftUrl(['logout'], [{ name: 'adminPanel', value: 1 }])
      : craftUrl(['logout'])
    const res = await fetch(url, {
      method: 'GET',
      credentials: 'include',
...
export const _getFetcher = async (apis, cookies = null) => {
  // [{name: url}, {name: url}]
  let results = {}
  try {
    await Promise.all(
      Object.entries(apis).map(async ([api, url]) => {
        ...
        if (res) results[api] = await res.json()
...
export const createReqObject = (originalData, finalData, deletedData) => {
  ...
  finalData.forEach((item) => {
    if (!item.hasOwnProperty('id')) {
      //if it doesnt have id its a POST
      ...
      if (!isEqual(originalItem, item)) requestObj.PUT.push(item) //if you find it and they're not equal its a PUT
    }
  })
