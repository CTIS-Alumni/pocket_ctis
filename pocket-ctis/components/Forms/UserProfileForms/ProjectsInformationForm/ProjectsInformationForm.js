...
const ProjectsInformationForm = ({data}) => {
    ...
    const transformFuncs = {
        projects: (newData) => {
            newData.projects = newData.projects.map((val) => {
                ...
                val.project_description = val.project_description ? val.project_description : null;
                replaceWithNull(val)
                return val;
            });
        },
        graduation_project: (newData) => {
            newData.graduation_project = newData.graduation_project.map((val) => {
                val.graduation_project_description = val.graduation_project_description ? val.graduation_project_description : null;
                val.visibility = val.visibility ? 1 : 0;
                replaceWithNull(val);
                return val;
            });
    ...
    const onSubmit = async (values) => {
        ...
        await Promise.all(Object.keys(dataAfterSubmit).map(async (key) => {
            const send_to_req = {};
                                        ...
                                        return (
                                            <>
                                                ...
                                                props.values.graduation_project.length > 0 ? (
                                                    props.values.graduation_project.map((project, index) => {
                                                        return (
                                                            <>
                                                                        ...
                                                                        <div style={{display: 'flex'}}>
                                                                            <div className={styles.removeBtnContainer}>
                                                                                <button
                                                                                    ...
                                                                                    type='button'
                                                                                    onClick={() => {
                                                                                        props.setFieldValue(`graduation_project[${index}].graduation_project_description`, '');
                                                                                    }}
                                                                                >
                                                                                    <XCircleFill
                                                                                        size={13}
